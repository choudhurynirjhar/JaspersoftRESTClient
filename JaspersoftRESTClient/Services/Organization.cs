...
namespace JaspersoftRESTClient
{
    class Organization
    {
        ...
        public bool Exists()
        {
            ...
            {
                request = WebRequest.Create("http://localhost:8080/jasperserver-pro/rest/organization/" + _organizationId) as HttpWebRequest;
                request.CookieContainer = cookieContainer;
                request.Method = "GET";
        ...
        public void Create(string organizationId)
        {
            if (_tanent != null)
            {
                ...
                if (response.StatusCode == HttpStatusCode.OK)
                {
                    ...
                    request = WebRequest.Create("http://localhost:8080/jasperserver-pro/rest/organization/") as HttpWebRequest;
                    request.CookieContainer = cookieContainer;
                    request.Method = "PUT";
